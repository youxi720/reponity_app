...
{
    const POST_PAGINATION_LIMIT = 5; // ページネーションの制限
    ...
    public function post_index(Request $request)
    {
        ...
        // 投稿とターゲットデータを取得
        $posts = $query->orderBy('updated_at', 'DESC')->paginate(self::POST_PAGINATION_LIMIT);
    ...
    {
        $posts = Post::where('user_id', Auth::id())->orderBy('updated_at', 'DESC')->paginate(self::POST_PAGINATION_LIMIT);
    ...
    {
        return $this->toggleLike($post, true);
    }

    // 「いいね」を削除
    public function unlike(Request $request, Post $post)
    {
        return $this->toggleLike($post, false);
    }

    private function toggleLike(Post $post, bool $isLike)
    {
        $user = Auth::user();

        if ($user && $user->likedPosts()->where('post_id', $post->id)->exists() !== $isLike) {
            $isLike ? $user->likedPosts()->attach($post->id) : $user->likedPosts()->detach($post->id);
            return response()->json(['message' => $isLike ? 'Liked successfully' : 'Unliked successfully'], 200);
        }

        return response()->json(['message' => $isLike ? 'Already liked' : 'Not liked'], $isLike ? 409 : 404);
    }
    ...
    public function likeshow()
    {
        ...
        })->orderBy('updated_at', 'DESC')->paginate(self::POST_PAGINATION_LIMIT);
